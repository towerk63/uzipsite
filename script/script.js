...
$('#loginmenu_mobile').click(function () {
  if ($('#nav_item_au_login_show').attr('class') == "nav-item-clone-mobile") {
    $('#nav_item_au_login_show').addClass('nav-item-clone-mobile_show');
    $('#nav_link_icon_login').addClass('fa-sort-up');
    $('#nav_link_icon_login').removeClass('fa-sort-down');
  } else {
    $('#nav_item_au_login_show').removeClass('nav-item-clone-mobile_show');
    $('#nav_link_icon_login').removeClass('fa-sort-up');
    $('#nav_link_icon_login').addClass('fa-sort-down');
  }
});
...
$("#registration").click(function () {
  $('#loginuz').addClass("login");
  $('#registrationuz').removeClass("login");
  $("#exampleModalScrollableTitle").text("registration");
});
$("#login").click(function () {
  $('#registrationuz').addClass("login");
  $('#loginuz').removeClass("login");
  $("#exampleModalScrollableTitle").text("login");
});
$conn = 1;
...
$("#sub_login").click(function () {
  $.post("server/login.php",
    ...
    function (data) {
      if (data != "The account has not been activated" && data != "Login failed") {
        $("#msg").html("Xin chào " + data);
        $("#loginmenu_mobile").html(data);
        $("#loginmenu").html(data);
        $('#msg').css('color', 'blue');
...
window.onload = function () {
  $.post("server/cookkie.php",
    function (data) {
      if (data == "ok") {
        $.post("server/checklogin.php",
          function (data) {
            $newdata = data + '&nbsp;<i id="nav_link_icon_login" class="fas fa-sort-down"></i>';
            $(".okdownload").attr('data-toggle', "");
            $("#loginmenu_mobile").html($newdata);
            $("#loginmenu").html(data + '&nbsp;<i class="fas fa-sign-out-alt"></i>');
          }
        );
      }
    }
...
$("#loginmenu").click(function () {
  if ($("#loginmenu").text() != "Login") {
    $.post("server/delcookkie.php",
      function () {
        $countlogin = 0;
        location.reload();
      }
...
$("#sub_regis").click(function () {
  ...
  $emailr = $('#emailr').val();
  $phonenbr = $('#phonenbr').val();
  $titler = $('#titler').val();
  $companyr = $('#companyr').val();
  $addr = $('#addr').val();
  function check(input, text) {
    text = "#" + text;
    if (input == "") {
      $(text).text("Not be empty");
      $active = 0;
    } else {
      $(text).text("");
    }
  ...
  check($fullnamer, "mfullnamer");
  check($usernamer, "musernamer");
  check($passwordr, "mpasswordr");
  check($cpasswordr, "mcpasswordr");
  check($emailr, "memailr");
  if ($passwordr != $cpasswordr) {
    $("#mcpasswordr").text("Confirm password incorrect");
    $("#mcpasswordr").css("color", "red");
    $active = 0;
  } else if ($cpasswordr != "") {
    $("#mcpasswordr").text("");
  }
  $checkemail = $emailr;
  $atposition = $checkemail.indexOf("@");
  $dotposition = $checkemail.lastIndexOf(".");
  if (($atposition < 1 || $dotposition < ($atposition + 2)
    || ($dotposition + 2) >= $checkemail.length) && $checkemail != "") {
    $("#memailr").text("Email invalidate");
    $("#memailr").css("color", "red");
    $active = 0;
  }
  if ($active == 1) {
    $.post("server/regis.php",
      {
        fullnamer: $fullnamer,
        usernamer: $usernamer,
        passwordr: $passwordr,
      ...
      function (data) {
        ...
        if (data == "username") {
          ...
          alert("Unable to register for an account");
        }
      }
    );
  }
...
$("#sendmail").click(function () {
  $.post("server/mailer.php",
    {
      usernamemail: $('#usernamemail').val(),
      emailmail: $('#emailmail').val(),
      mess: $('#mess').val()
    },
    function (data) {
      $("#msgmail").html(data);
      $("#msgmail").css('color', 'blue');
    }
  );
});
$(document).ready(function () {
  $.getJSON("server/getfile.php", function (data) {
    ...
    $("#linkdriver1").attr("href", $_slt[0]);
    $("#linkdriver2").attr("href", $_slt[1]);
    $("#linkdriver3").attr("href", $_slt[2]);
    $("#linkdriver4").attr("href", $_slt[3]);
    for ($i = 0; $i < 4; $i++) {
      console.log($_slt[$i]);
    }
  });
